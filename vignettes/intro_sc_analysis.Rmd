...
We can also, visualize how the cells are distributed under different discrete meta.data variables:
...
```{r fake-comparison-truth, include=TRUE,echo=TRUE,eval=TRUE}
lung_data@meta.data %>%
  select(Major.cell.type, seurat_clusters) %>% 
  table()
```

```{r comparison-truth, include=FALSE, echo = FALSE, eval = TRUE}
lung_data@meta.data %>%
  select(Major.cell.type, seurat_clusters) %>% 
  table() %>% 
  knitr::kable() %>% 
